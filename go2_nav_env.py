...
if GENESIS_PATH is None:
    ...
    print(f"[go2_nav_env] Genesis repo not found. Checked: {checked_paths}")
    print("[go2_nav_env] Please clone Genesis repository and run again.")
    raise FileNotFoundError("Genesis not found")
...
class Go2NavEnv:
    def __init__(self, num_envs, env_cfg, obs_cfg, reward_cfg, command_cfg, show_viewer=False, nav_map=None):
        ...
        if nav_map is not None:
            start_x, start_y = nav_map.get_simulated_start_world()
            self.env_cfg["base_init_pos"] = [start_x, start_y, self.env_cfg["base_init_pos"][2]]
        ...
        # Add navigation walls if nav_map provided
        if nav_map is not None:
            self._add_walls(nav_map)
    ...
    def _add_walls(self, nav_map) -> None:
        """Add wall entities to the scene based on navigation map."""
        wall_height = 0.8
        for gx in range(nav_map.width_cells):
            for gy in range(nav_map.height_cells):
                if nav_map.raw_grid_np[gx, gy] == 1:
                    wx, wy = nav_map.grid_to_world(gx, gy)
                    self.scene.add_entity(
                        gs.morphs.Box(
                            size=(nav_map.cell_size_m * 0.9,
                                  nav_map.cell_size_m * 0.9,
                                  wall_height),
    ...
    def set_nav_command(self, vx: float, vy: float, yaw_rate: float) -> None:
        """Set navigation command for the robot."""
        self.commands[:, 0] = vx
        self.commands[:, 1] = vy
        self.commands[:, 2] = yaw_rate

    def _resample_commands(self, envs_idx) -> None:
        """Override to no-op to disable random command resampling."""
        pass

    def get_robot_world_pose(self) -> Tuple[float, float, float]:
        """Get robot world pose (x, y, yaw)."""
        import torch
        pos = self.robot.base_link.get_pos()
        quat = self.robot.base_link.get_quat()
        w = quat[0, 0]; x = quat[0, 1]
        y = quat[0, 2]; z = quat[0, 3]
        yaw = torch.atan2(2*(w*z + x*y), 1 - 2*(y*y + z*z))
        return pos[0,0].item(), pos[0,1].item(), yaw.item()
