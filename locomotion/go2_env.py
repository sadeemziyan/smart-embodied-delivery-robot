...
class Go2Env:
    def __init__(self, num_envs, env_cfg, obs_cfg, reward_cfg, command_cfg, show_viewer=True):
        ...
        # foot links and contact sensors
        self.foot_link_names = ["FL_calf", "FR_calf", "RL_calf", "RR_calf"]
        self.foot_links = [self.robot.get_link(name) for name in self.foot_link_names]

        self.contact_sensors = [
            self.scene.add_sensor(
        ...
        # contact and air time buffers
        self.contact_forces = torch.zeros((self.num_envs, 4, 3), dtype=gs.tc_float, device=gs.device)
    ...
    def step(self, actions):
        ...
        # read contact forces from sensors
        self.contact_forces = torch.stack(
            [sensor.read() for sensor in self.contact_sensors], dim=1
        )  # shape: (num_envs, 4, 3)
        # compute reward
        self.rew_buf.zero_()
    ...
    def _reward_feet_air_time(self):
        # Reward long steps
        contact = self.contact_forces[:, :, 2] > 1.0
    ...
    def _reward_stumble(self):
        ...
        return torch.any(
            torch.norm(self.contact_forces[:, :, :2], dim=2) >
            5 * torch.abs(self.contact_forces[:, :, 2]),
            dim=1
        ).float()
    ...
    def _reward_feet_contact_forces(self):
        ...
        return torch.sum(
            (torch.norm(self.contact_forces, dim=-1) - self.reward_cfg["max_contact_force"]).clip(min=0.),
            dim=1
        )